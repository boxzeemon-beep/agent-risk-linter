...
import { SEVERITY_RANK, type Finding, type LinterConfig, type Rule, type RuleMatch, type ScanExecution, type ScanOptions, type ScanSummary, type SourceFile } from "./types.js";
...
function locationAt(content: string, index: number): Location {
...
function fingerprint(file: SourceFile, rule: Rule, match: RuleMatch): string {
...
function findingFromMatch(file: SourceFile, rule: Rule, match: RuleMatch): Finding {
...
function disabledRulesForFile(config: LinterConfig, relativePath: string, commandLineDisabled: readonly string[]): Set<string> {
  ...
  for (const override of config.overrides) {
    if (matchesAnyGlob(relativePath, override.files)) {
      for (const ruleId of override.disableRules) disabled.add(ruleId.toUpperCase());
    }
  }
  return disabled;
}

function summarize(findings: readonly Finding[]): ScanSummary {
...
export async function scanProject(options: ScanOptions): Promise<ScanExecution> {
  const target = path.resolve(options.root);
  const stats = await lstat(target);
  if (stats.isSymbolicLink()) throw new Error("Refusing to follow a symbolic-link scan target.");
  ...
  for (const ruleId of allConfiguredDisabled) {
    if (!knownRuleIds.has(ruleId)) diagnostics.push({ level: "warning", code: "UNKNOWN_RULE", message: `Disabled rule does not exist: ${ruleId}` });
  }
  ...
  for (const file of collection.files) {
    ...
    for (const rule of RULES) {
      if (!rule.appliesTo.includes(file.kind) || disabled.has(rule.id)) continue;
      for (const match of rule.detect({ file })) findings.push(findingFromMatch(file, rule, match));
    }
  }
...
export function addFindings(execution: ScanExecution, additional: Finding[]): ScanExecution {
  const findings = deduplicate([...execution.result.findings, ...additional]).sort(
    (left, right) => SEVERITY_RANK[right.severity] - SEVERITY_RANK[left.severity] || left.file.localeCompare(right.file) || left.line - right.line,
  );
  return {
    ...execution,
    result: {
