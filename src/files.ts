...
export function classifyFile(relativePath: string): FileKind {
  ...
  if (SCRIPT_EXTENSIONS.has(extension) || SPECIAL_TEXT_NAMES.has(base)) return "script";
  return "document";
}
...
function hasUnsafePathControls(relativePath: string): boolean {
...
export async function collectFiles(options: CollectOptions): Promise<CollectResult> {
  ...
  if (rootStats.isFile()) {
    ...
    if (!shouldRead(relativePath)) return { files, diagnostics };
  ...
  while (pending.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (DEFAULT_IGNORED_DIRECTORIES.has(entry.name.toLowerCase()) || matchesAnyGlob(`${relativePath}/`, options.ignore)) continue;
        pending.push(absolutePath);
        continue;
      }

      if (!entry.isFile() || !shouldRead(relativePath) || matchesAnyGlob(relativePath, options.ignore)) continue;
