...
function parseScanArguments(args) {
    ...
    while (args.length > 0) {
        ...
        if (argument === "--format") {
            ...
            const value = requireValue(args, argument);
            if (value !== "none" && !isSeverity(value))
...
function thresholdReached(failOn, findings) {
    if (failOn === "none")
        return false;
...
async function runRules(args) {
    ...
    if (args.some((argument) => argument.startsWith("-") && argument !== "--json"))
        throw new Error("rules supports only --json.");
    if (ruleId) {
...
async function runScan(args) {
    ...
    if (parsed.semanticPreview) {
        ...
        if (!parsed.quiet)
            process.stderr.write(`Wrote redacted semantic payload preview to ${parsed.semanticPreview}. No network request was made.\n`);
    }
    if (parsed.semantic) {
        const apiKey = process.env.OPENAI_API_KEY;
        if (!apiKey)
            throw new Error("--semantic requires OPENAI_API_KEY. The key is read from the environment and is never written to output.");
        const model = parsed.model ?? execution.config.semantic.model;
        if (!parsed.quiet)
            process.stderr.write(`Opt-in semantic review: sending at most ${maxSemanticChars} redacted characters to OpenAI using ${model}.\n`);
    ...
    if (parsed.output) {
        ...
        if (!parsed.quiet)
            process.stderr.write(`Wrote ${parsed.format} report to ${parsed.output}.\n`);
    }
    else {
