...
function patternRule(definition, patterns) {
    return {
        ...definition,
        detect({ file }) {
            const matches = [];
...
function parsePackageJson(context) {
    if (path.posix.basename(context.file.relativePath.toLowerCase()) !== "package.json")
        return undefined;
    try {
        const value = JSON.parse(context.file.content);
        ...
        detect(context) {
            ...
            for (const sectionName of ["dependencies", "devDependencies", "optionalDependencies"]) {
                ...
                for (const [name, specifier] of Object.entries(section)) {
                    if (typeof specifier !== "string" || !/^(?:https?:|git(?:\+|:)|github:)/i.test(specifier))
                        continue;
...
function hardcodedCredentialRule() {
...
function workflowCombinationRule() {
    return {
        id: "CI003",
        ...
        detect({ file }) {
            ...
            if (!event || !untrustedHead)
                return [];
            return [{ index: event.index, length: event[0].length }];
        },
    };
}
function skillMetadataRule() {
    return {
        id: "META001",
        ...
        detect({ file }) {
            if (path.posix.basename(file.relativePath).toLowerCase() !== "skill.md")
                return [];
            const frontmatter = /^---\s*\r?\n([\s\S]*?)\r?\n---/.exec(file.content);
            if (!frontmatter)
                return [{ index: 0, length: Math.min(file.content.length, 1) }];
