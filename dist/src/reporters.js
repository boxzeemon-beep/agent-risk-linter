...
export function renderText(result, options = {}) {
    ...
    if (result.findings.length === 0) {
        ...
        for (const finding of result.findings) {
            lines.push("", `${severityLabel(finding, color)} ${finding.ruleId}  ${sanitizeTerminalText(finding.file)}:${finding.line}:${finding.column}`, `  ${finding.title}`, `  ${finding.description}`);
            if (finding.excerpt)
                lines.push(`  > ${finding.excerpt}`);
            lines.push(`  Fix: ${finding.remediation}`);
            if (finding.source === "semantic")
                lines.push("  Source: opt-in semantic review; verify manually.");
        }
    }
    ...
    if (warnings.length > 0) {
        ...
        for (const diagnostic of warnings)
            lines.push(`  [${diagnostic.code}] ${diagnostic.file ? `${sanitizeTerminalText(diagnostic.file)}: ` : ""}${sanitizeTerminalText(diagnostic.message)}`);
    }
    return `${lines.join("\n")}\n`;
}
